...
import {
  LogOut, Search, QrCode, Plus, BarChart3, Ticket, CheckCircle, CreditCard, Banknote, Loader2, X, ScanLine
} from "lucide-react";
...
type FilterType = "all" | "Unused" | "Redeemed" | "Online" | "Cash";
  ...
  const handleScanResult = async (scannedText: string) => {
    ...
    try {
      ...
      if (data.status === "Redeemed") {
        setScanResult({ coupon: data as Coupon, message: "Already Redeemed!", valid: false });
      } else {
        setScanResult({ coupon: data as Coupon, message: "Valid — Unused", valid: true });
      }
    } catch {
      ...
      <main className="max-w-6xl mx-auto p-4 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "Online", value: stats.online, icon: CreditCard, color: "bg-accent/20 text-accent-foreground" },
        ...
        {showManual && (
          <div className="bg-card border rounded-xl p-5 space-y-4">
            ...
            {manualCoupon ? (
              <div>
                <CouponCard
                  couponId={manualCoupon.coupon_id}
                  name={manualCoupon.name}
                  phone={manualCoupon.phone}
                  paymentType={manualCoupon.payment_type}
                  createdAt={manualCoupon.created_at}
                />
                <Button variant="outline" className="mt-4 w-full" onClick={() => setManualCoupon(null)}>
                  Generate Another
                </Button>
              </div>
            ) : (
                  ...
                  <tr key={c.id} className="border-b hover:bg-muted/50 transition-colors">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        c.payment_type === "Online" ? "bg-accent/20 text-accent-foreground" : "bg-secondary text-secondary-foreground"
                      }`}>
                        {c.payment_type}
                      </span>
                    </td>
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        c.status === "Unused" ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"
                      }`}>
                        {c.status}
                      </span>
                    </td>
